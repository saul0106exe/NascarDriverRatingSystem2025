...
def analyze_all_tracks(tracks_metadata):
    ...
    # scaler to 100
    category_cols = ['OVR', 'SSW', 'SW', 'INT', 'S.INT', 'C', 'RC']
    for col in category_cols:
        if col in final_df.columns:
            # non numeric to NaN
            numeric_ratings = pd.to_numeric(final_df[col], errors='coerce')
            if not numeric_ratings.isnull().all():
                highest_rating = numeric_ratings.max()
                adjustment = 100 - highest_rating
                final_df[col] = numeric_ratings + adjustment

    final_df = final_df.sort_values(by='OVR', ascending=False)
